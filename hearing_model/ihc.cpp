...
{
    const int totalstim = (int)floor(reptime / binsize + 0.5);
    
    if (species == CAT)
        validate_parameter(cf, 124.9, 40.1e3, "cf");
    else
        validate_parameter(cf, 124.9, 20.1e3, "cf");

    validate_parameter(nrep, size_t{ 1 }, std::numeric_limits<size_t>::max(), "nrep");
    validate_parameter(reptime, sound_wave.size() * binsize, std::numeric_limits<double>::infinity(), "reptime");
    validate_parameter(cohc, 0., 1., "cohc");
    validate_parameter(cihc, 0., 1., "cihc");
         
    std::vector<double> px = sound_wave;
    px.resize(totalstim, 0.0);    
    std::vector<double> ihcout(totalstim * nrep);
    
    IHCAN(px.data(), cf, static_cast<int>(nrep), binsize, totalstim, cohc, cihc, static_cast<int>(species), ihcout.data());
    return ihcout;
}
    ...
    double cohc, double cihc, int species, double* ihcout)
{
    ...
    for (n = 0; n < totalstim; n++) /* Start of the loop */
    {
        ...
        wbout = pow((tauwb / TauWBMax), wborder) * wbout1 * 10e3 * std::max(1., cf / 5e3);
    ...
    delaypoint = std::max(0, (int)ceil(delay / tdres));
...
double Get_tauwb(double cf, int species, int order, double* taumax, double* taumin)
{
    ...
    else if (species == 2) /* human Q10 values from Shera et al. (PNAS 2002) */
    {
        Q10 = pow((cf / 1000), 0.3) * 12.7 * 0.505 + 0.2085;
    }
    else/* human Q10 values from Glasberg & Moore (Hear. Res. 1990) */
    {
...
double C1ChirpFilt(double x, double tdres, double cf, int n, double taumax, double rsigma)
{
    ...
    if (n == 0)
    {
        ...
        for (r = 1; r <= order_of_pole; r++)
            C1gain_norm = C1gain_norm * (pow((CF - p[r].imag()), 2) + p[r].real() * p[r].real());
    };
    ...
    for (i = 1; i <= half_order_pole; i++)
    {
        ...
        dy = C1input[i][1] * (fs_bilinear - rzero) - 2 * rzero * C1input[i][2] - (fs_bilinear + rzero) * C1input[i][3] + 2 * C1output[i][1] * (fs_bilinear * fs_bilinear - preal * preal - pimg * pimg) - C1output[i][2] * ((fs_bilinear + preal) * (fs_bilinear + preal) + pimg * pimg);
...
double C2ChirpFilt(double xx, double tdres, double cf, int n, double taumax, double fcohc)
{
    ...
    if (n == 0)
    {
        ...
        C2gain_norm = 1.0;
        for (r = 1; r <= order_of_pole; r++)
            C2gain_norm = C2gain_norm * (pow((CF - p[r].imag()), 2) + p[r].real() * p[r].real());
    };
    ...
    for (i = 1; i <= half_order_pole; i++)
    {
        ...
        dy = C2input[i][1] * (fs_bilinear - rzero) - 2 * rzero * C2input[i][2] - (fs_bilinear + rzero) * C2input[i][3] + 2 * C2output[i][1] * (fs_bilinear * fs_bilinear - preal * preal - pimg * pimg) - C2output[i][2] * ((fs_bilinear + preal) * (fs_bilinear + preal) + pimg * pimg);
...
double WbGammaTone(double x, double tdres, double centerfreq, int n, double tau, double gain, int order)
{
    ...
    if (n == 0)
    {
        ...
        for (i = 0; i <= order; i++)
        {
            wbgtfl[i] = Complex(0.0); // compmult(0, compexp(0));
            wbgtf[i] = Complex(0.0); //compmult(0, compexp(0));
        }
    }
    ...
    c1LP = (dtmp - 1) / (dtmp + 1);
    c2LP = 1.0 / (dtmp + 1);
    wbgtf[0] = Complex(cos(wbphase), sin(wbphase)) * x; /* FREQUENCY SHIFT */
    ...
    /*auto tmp = Complex(cos(-wbphase), sin(-wbphase));
    
    Complex(
        (tmp.real() * wbgtf[order].real()) - (tmp.imag() * wbgtf[order].imag()),
        (tmp.real() * wbgtf[order].imag()) + (tmp.imag() * wbgtf[order].real())
    ).real()*/


    /* FREQ SHIFT BACK UP */
    out = (cos(-wbphase) * wbgtf[order].real()) - (sin(-wbphase) * wbgtf[order].imag());
        
    //REAL(compprod(compexp(-wbphase), wbgtf[order])); 
