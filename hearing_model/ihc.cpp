...
namespace ihc
{
	...
	struct MiddleEarFilter
	{
		...
		MiddleEarFilter(const double time_resolution, const Species species): m{}, mey1{}, mey2{}, mey3{}, px_cache{},
		                                                                      i{0}
		{
	...
	struct WideBandGammaToneFilter
	{
		...
		WideBandGammaToneFilter(
			...
			previous_gain(0.0)
		{
			const double gain = max(min(52.0 / 2.0 * (tanh(2.2 * log10(cf / 0.6e3) + 0.15) + 1.0), 60.0), 15.0);
		...
		double operator()(const double me_out)
		{
			phase += delta_phase;
		...
		void shift_poles(const double tau_c1, const size_t n)
		{
	...
	struct BoltzmanFilter
	{
		...
		double operator()(const double wb_out) const
		{
	...
	struct LowPassFilter
	{
		...
		LowPassFilter(const double time_resolution, const double fc): ohc{}, ohc_last{}
		{
	...
	struct PostOhcFilter
	{
		...
		PostOhcFilter(const double cohc, const double bm_tau_min, const double bm_tau_max, const double asym)
			: cohc(cohc),
	...
	struct LogarithmicTransductionFunction
	{
		...
		LogarithmicTransductionFunction(const double slope, const double asym):
			slope(slope),
			asymptote(asym),
	...
	struct ChirpFilter
	{
		...
		ChirpFilter(const double time_resolution, const double cf_in, const double bm_tau_max, const bool c1 = true) :
			time_resolution(time_resolution),
			cf(cf_in * TWO_PI),
			tau_max(bm_tau_max),
			init_phase(0.0),
		...
		void fill_phase_array(const double r_sigma, const bool multiply = false)
		{
		...
		double operator()(const double me_out, const double r_sigma)
		{
